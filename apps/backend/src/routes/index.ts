export { default as authRoutes } from "./auth.route.js";
export { default as customerRoutes } from "./customer.route.js";
export { default as petRoutes } from "./pet.route.js";
export { default as productRoutes } from "./product.route.js";
export { default as serviceRoutes } from "./service.route.js";
export { default as dashboardRoutes } from "./dashboard.route.js";
export { default as medicalHistoryRoutes } from "./medical-history.route.js";
export { default as transactionRoutes } from "./transaction.route.js";
