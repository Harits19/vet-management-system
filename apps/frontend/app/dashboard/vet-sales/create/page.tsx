...
export default function CreateVetSalePage() {
      ...
      .then(async ([c, s, m]) => {
        ...
        setMedicines(
          m.data.map((x: any) => ({
            _id: x._id,
            name: x.product?.name,
            price: x.pricing?.selling ?? 0,
            cost: x.pricing?.cost ?? 0,
            type: "physical",
          }))
        );
    ...
    setCart((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          quantity: 1,
          pricing: {
            cost: prod.pricing.cost || 0,
            selling: prod.pricing.selling,
            total: prod.pricing.selling,
          },
    ...
    setCart((prev) =>
      prev.map((i) =>
        i.product._id === productId
          ? { ...i, quantity: qty, pricing: { ...i.pricing, total: i.pricing.selling * qty } }
          : i
      )
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!values.diagnosis) {
        msg.error("Diagnosis wajib diisi");
        setSubmitting(false);
        return;
      }
    ...
    <div>
        ...
        <Form form={form} layout="vertical">
          ...
          {selectedPet && (
            <Card size="small" title="Riwayat Rekam Medis" style={{ marginBottom: 16 }}>
              {mhRecords.length === 0 ? (
                <Text type="secondary">Belum ada riwayat</Text>
              ) : (
                <Space orientation="vertical" style={{ width: "100%" }}>
                  {mhRecords.slice(0, 3).map((r: any) => (
                    <Row key={r._id} gutter={8}>
                      ...
                      <Col xs={24} sm={12}>
                        <Text ellipsis>{r.diagnosis}</Text>
                      </Col>
                      <Col span={6}>
                        <Text type="secondary">
          ...
          <Form.Item
            name="diagnosis"
            label="Diagnosis"
            rules={[{ required: true, message: "Diagnosis wajib" }]}
          >
      ...
      <Card title="Item Transaksi" style={{ marginTop: 16 }}>
        <Space orientation="vertical" style={{ width: "100%" }}>
          <Select
            showSearch
            placeholder="Tambah jasa dokter..."
            style={{ width: "100%" }}
          ...
          {cart.map((item) => (
            <Row key={item.product._id} gutter={8} align="middle" style={{ marginTop: 4 }}>
              <Col flex="auto">
                ...
                <Tag
                  color={item.product.type === "service" ? "blue" : "green"}
                  style={{ marginLeft: 8 }}
                >
                  {item.product.type === "service" ? "Jasa" : "Obat"}
                </Tag>
              </Col>
              <Col>
                <Input
              ...
              {item.product.type === "physical" && (
                <Col>
                  <Input
                    placeholder="Dosis"
                    value={item.dosage || ""}
                    onChange={(e) => updateDosage(item.product._id, e.target.value)}
                    style={{ width: 100 }}
                  />
                </Col>
              )}
              <Col>
                <Text>{formatPrice(item.pricing.total)}</Text>
              </Col>
              <Col>
                <Button
                  size="small"
                  danger
                  onClick={() =>
                    setCart((prev) => prev.filter((i) => i.product._id !== item.product._id))
                  }
                >
                  X
                </Button>
