...
- **Frontend**: `/dashboard/attendance` (status hari ini + panel metode + tombol Masuk/Pulang + riwayat + link cetak QR utk superadmin), `/dashboard/attendance/register` (daftar wajah; daftar ulang = ganti; alert saat mode wajah nonaktif), `/dashboard/attendance/qr` (cetak + generate ulang QR — superadmin). Menu "Absensi" (Clock) untuk semua role non-superadmin.
