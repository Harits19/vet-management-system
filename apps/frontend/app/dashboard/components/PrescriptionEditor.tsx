...
export default function PrescriptionEditor({ items, onChange, options, loading, onSearch }: PrescriptionEditorProps) {
  ...
  const usageTimeOptions = Array.from(new Set([...USAGE_TIME_SEEDS, ...distinctValues((l) => l.usageTime)]));
  const usageInstructionOptions = Array.from(new Set([...USAGE_INSTRUCTION_SEEDS, ...distinctValues((l) => l.usageInstruction)]));
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
      {items.map((line) => {
        ...
        const drugOptions = options.map((o) => ({ value: o._id, label: `${o.name} - ${formatPrice(o.selling)}` }));
        if (line.productId && !options.some((o) => o._id === line.productId) && line.name) {
          drugOptions.push({ value: line.productId, label: `${line.name} - ${formatPrice(line.price)}` });
        }
        ...
        return (
          <Space key={line._key} direction="vertical" style={{ width: "100%", border: `1px solid ${token.colorBorderSecondary}`, borderRadius: 8, padding: 8 }}>
            <Row gutter={8} align="middle">
              ...
              <Col>
                <InputNumber min={1} value={line.quantity} onChange={(v) => updateLine(line._key, { quantity: v ?? 1 })} style={{ width: 60 }} placeholder="Qty" />
              </Col>
              <Col>
            ...
            <Row gutter={8}>
              ...
              <Col xs={24} sm={8}>
                <DistinctTagSelect value={line.usageInstruction} options={usageInstructionOptions} placeholder="Instruksi (mis. tab, cth)" onChange={(v) => updateLine(line._key, { usageInstruction: v })} />
              </Col>
              <Col xs={24} sm={8}>
                <DistinctTagSelect value={line.usageNote} options={usageNoteOptions} placeholder="Catatan Penggunaan (mis. setelah makan)" onChange={(v) => updateLine(line._key, { usageNote: v })} />
              </Col>
            </Row>
            <Row gutter={8}>
              <Col xs={24} sm={8}>
