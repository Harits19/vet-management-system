...
export async function getStatus(userId: string) {
  ...
  return {
    ...
    today,
    todayIn: todayIn ? { type: todayIn.type, timestamp: todayIn.timestamp } : null,
    todayOut: todayOut ? { type: todayOut.type, timestamp: todayOut.timestamp } : null,
  };
}
...
export async function checkIn(userId: string, role: string, input: CheckInInput) {
  ...
  if (cfg.locationEnabled) {
    ...
    distanceFromOffice = haversineMeters(lat, lng, cfg.officeLat as number, cfg.officeLng as number);
    if (distanceFromOffice > cfg.radiusMeters) {
      throw Object.assign(
        new Error(
          `Di luar area kantor (jarak ${Math.round(distanceFromOffice)} m dari titik kantor, maks ${cfg.radiusMeters} m)`
        ),
        { status: 400 }
      );
    }
  ...
  const faceDistance = euclideanDistance(descriptor, user.faceDescriptor);
