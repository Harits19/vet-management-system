...
import { listProducts, getProduct, createProduct, updateProduct, deleteProduct, searchProductsByCode } from "../services/product.service.js";
...
export async function getServices(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const filter = productFilterSchema.parse({ ...req.query, type: "service" });
    const result = await listProducts(filter);
    res.json({ success: true, data: result.data, meta: { page: result.page, limit: result.data.length, total: result.total, totalPages: Math.ceil(result.total / filter.limit) } });
  } catch (err) { next(err); }
}

export async function getPhysical(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const filter = productFilterSchema.parse({ ...req.query, type: "physical" });
    const result = await listProducts(filter);
    res.json({ success: true, data: result.data, meta: { page: result.page, limit: result.data.length, total: result.total, totalPages: Math.ceil(result.total / filter.limit) } });
  } catch (err) { next(err); }
}
