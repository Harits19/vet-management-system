...
export async function syncInventory(file: Buffer): Promise<IProductDoc[]> {
...
export async function syncService(file: Buffer): Promise<IServiceDoc[]> {
...
export function parseAge(value?: string): IPetDocInitialAge | undefined {
...
export async function syncPatient(file: Buffer): Promise<IPetDoc[]> {
  ...
  for (const patient of data) {
    ...
    patients.push({
      ...
      notes: patient.notes,
      updatedAt: new Date(),
      initialAge: parseAge(patient.age),
      breed: patient.breed,
...
export async function syncMedicalHistory(
  file: Buffer,
): Promise<IMedicalHistoryDoc[]> {
